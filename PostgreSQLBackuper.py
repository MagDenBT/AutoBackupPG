...
def setParam(newArgs=None, inLowerCase = False):

    global args
    _generateParams(args)
...
def _generateParams(args):
    ...
    args.update({'backuper': None})
    args.update({'tempPath': None})
    args.update({'logPath': None})
...
def _addParams(args):
    ...
    if args.get('backuper') == None or args.get('backuper') == '':
        args.update({'backuper': args["postgresqlIsntancePath"] + 'bin\\pg_basebackup.exe'})

    if args.get('tempPath') == None or args.get('tempPath') == '':
        args.update({'tempPath': './temp'})  # The path to the temporary directory for full backup

    if args.get('logPath') == None or args.get('logPath') == '':
        args.update({'logPath': './PostgreSQLBackuperLogs'})  # The path to the script logs

    if args.get('URL') == None or args.get('URL') == '':
