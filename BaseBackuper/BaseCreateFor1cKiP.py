...
class BaseCreateFor1cKiP(BaseScenario):
    ...
    def _real(self):
        ...
        try:
            global_logger.info(message="Запущен процесс создания бэкапа файловой базы 1с")
            manager.create_backup(write_to_log_file, raise_exception=True)
            global_logger.info(message="Бэкап файловой базы 1с сделан")
        except Exception as e:
            error = str(e)
            global_logger.error(message=f"Не удалось создать бэкап файловой базы 1с.Причина - {error}. {traceback.format_exc()}'")
            raise SACError(24, f'Не удалось создать бэкап файловой базы 1с - {error}')
